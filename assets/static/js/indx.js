var ua = navigator.userAgent.match(/\sEdg\w\//), // For some reason Element.animate() doesn't work on Edge for mobile 
    ...
    }, notify = (text, type = 1, ms = 5000) => {
        text = text ? text.replace(/</g, '&lt;').replace(/>/g, '&gt;') : "Hello world";
...
onload = () => {
    ...
    document.querySelectorAll('.closeBtn, .ov').forEach(el => el.addEventListener('click', closeLinksBox));

    document.querySelectorAll('.menu ul li').forEach(el => {
        el.addEventListener('click', e => {
            let a = e.target.querySelector('a');
            if (a && a.getAttribute('href')) {
                if (a.getAttribute('target') && a.getAttribute('target') === '_blank') open(a.href);
    ...
    document.querySelector('.textBtn').addEventListener('click', () => {
        if (document.body.classList.contains('text'))
            ...
            // uploadBtn.style.display = 'inherit';
            // uploadBtn.animate([
            //     { pacity: 1, offset: 0 },
            //     { opacity: 0, offset: 1 },
            // ], {
            //     duration: 1000,
            //     easing: 'ease'
            // }).onfinish = () => uploadBtn.style.removeProperty('display');
