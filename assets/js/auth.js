var closeBtn = (el) => {
    try { el.animate({ top: '-50px', opacity: '0' }, { duration: 500, easing: 'cubic-bezier(.68, -0.55, .27, 1.55)' }).onfinish = () => el.remove(); }
    catch { };
}, notify = (text, type, ms) => {
...
window.onload = () => {
    ...
    document.querySelector('.login').addEventListener('click', () => {
        if (!input.value) return input.parentElement.animate([
